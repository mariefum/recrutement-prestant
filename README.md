...
Deux chemins complémentaires, **sans aucun backend ni variable d'environnement** :
...
1. **Candidature à une offre → ATS Beetween.** Chaque offre porte un champ `applyUrl`
   ...
   création de fiche candidat sont gérés nativement par Beetween.
2. **Candidature spontanée / contact → `mailto:`.** Les formulaires (candidature spontanée
   sur `/offres-emploi` et `/contact`, formulaire court sur les CTA) ouvrent au clic le
   logiciel de messagerie de l'utilisateur, **pré-rempli vers `sandy.miranda@prestant.com`**
   (sujet + champs du formulaire). Le candidat joint son CV directement dans l'email.

> L'adresse de destination est celle de `contact.email` dans `src/data/site.ts`.
> `mailto:` ne peut pas joindre de fichier automatiquement : le formulaire de candidature
> invite donc à attacher le CV dans l'email qui s'ouvre.
...
- [x] **Formulaires spontané/contact** : envoi par `mailto:` vers `sandy.miranda@prestant.com` (aucune config).
